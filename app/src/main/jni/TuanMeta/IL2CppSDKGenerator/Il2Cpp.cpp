...
#include "Il2Cpp.h"
#define IL2CPP__TAG "TuanMeta—Il2CppSdk"
...
int Il2Cpp::Attach(const char *libname) {
	...
	il2cpp_string_new_utf16 = (Il2CppString *(*)(const wchar_t *, int32_t)) get_export_function(libname, "il2cpp_string_new_utf16");
...
void *Il2Cpp::GetClass(const char *image, const char *namespaze, const char *clazz) {
	...
	if(classes.size() > 1)
	{
		...
		if (Il2cppDebug) IL2CPP_LOGE("Can't find subclass %s in class %s!", classes[1].c_str(), classes[0].c_str());
		return 0;
	}
			...
			for (int i = 0; i < argsCount; i++) {
				...
				if (arg) {
					...
					if (strcmp(tname, args[i]) == 0) {
						...
						if (Il2cppDebug) IL2CPP_LOGE("Argument at index %d didn't matched requested argument!\r\n\tRequested: %s\r\n\tActual: %s\r\nSkipping function...", i, args[i], tname);
						score = 0;
goto skip;
					}
				}
		...
		if(score == argsCount)
		{
			if (Il2cppDebug) IL2CPP_LOGE("Found matched function!");
	...
	if (Il2cppDebug) IL2CPP_LOGE("Cannot find function %s in class %s!", name, clazz);
	return 0;
}
